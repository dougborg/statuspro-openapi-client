# uv Package Manager Guide for StatusPro OpenAPI Client

This document provides common uv commands for working with the StatusPro OpenAPI Client
project.
...
# - statuspro-openapi-client (main package, current directory)
# - statuspro-mcp-server (MCP server package, separate directory)
```
...
```toml
[tool.uv.workspace]
members = [".", "statuspro_mcp_server"]
```

This allows both packages to share a unified lock file (`uv.lock`) and ensures version
compatibility. See [ADR-010](../adr/0010-statuspro-mcp-server.md) for the architectural
rationale.
...
uv run pytest tests/test_statuspro_client.py  # Specific file
...
# Quick import test
uv run python -c "from statuspro_public_api_client import StatusProClient; print('✅ Import successful')"
```
...
# Run a specific test
uv run pytest tests/test_statuspro_client.py::TestStatusProClient::test_client_initialization -v

# Check test coverage for specific module
uv run pytest tests/ --cov=statuspro_public_api_client.statuspro_client --cov-report=term-missing
```
