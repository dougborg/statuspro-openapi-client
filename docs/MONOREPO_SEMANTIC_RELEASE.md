...
1. **statuspro-openapi-client** - The main Python API client
1. **statuspro-mcp-server** - The Model Context Protocol server
...
- **statuspro-openapi-client**: `0.23.0` (tracks main client)
- **statuspro-mcp-server**: `0.1.0a1` (alpha release)
...
- **statuspro-openapi-client**: Publishes from `release-client` job
- **statuspro-mcp-server**: Publishes from `release-mcp` job
...
1. **Update**: Updates `statuspro-openapi-client>=X.Y.Z` in
   `statuspro_mcp_server/pyproject.toml`
...
git add statuspro_mcp_server/src/statuspro_mcp/tools/inventory.py
...
git add statuspro_public_api_client/helpers/products.py
...
git add statuspro_public_api_client/helpers/inventory.py
...
- **MCP Server**: `statuspro_mcp_server/CHANGELOG.md`
...
```bash
uv run semantic-release version --print --no-commit --no-tag --no-push \
  -c statuspro_mcp_server/pyproject.toml
```
...
1. Updates the `statuspro-openapi-client>=X.Y.Z` dependency in
   `statuspro_mcp_server/pyproject.toml`
...
1. Manually update `statuspro_mcp_server/pyproject.toml`
