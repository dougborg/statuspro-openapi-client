...
1. **Set up environment variables**
   ...
   ```bash
   cp .env.example .env
   # Edit .env with your StatusPro API credentials for testing
   ```
...
uv run poe test tests/test_statuspro_client.py
...
- Files in `statuspro_public_api_client/` → use `(client)` scope
- Files in `statuspro_mcp_server/` → use `(mcp)` scope
...
├── statuspro_public_api_client/    # Main package
│   ├── __init__.py
│   ├── statuspro_client.py         # Main client implementation
...
The OpenAPI client is automatically generated from `statuspro-openapi.yaml` using
...
- `statuspro_client.py` - Our resilient client implementation
...
- Merging a PR to `main` updates an aggregated release PR; merging _that_ PR is what
  actually cuts the release (tags + draft GitHub Releases, then publish via
  tag-triggered CI)
