...
The StatusPro API uses cursor-based pagination with Link headers:
...
1. Fetch first page (default limit: 250, StatusPro's max for efficiency)
...
```python
client = StatusProClient(max_pagination_pages=200)
```
...
- [AutoPaginationTransport Implementation](../../statuspro_public_api_client/statuspro_client.py)
- [StatusPro API Pagination Documentation](https://help.orderstatuspro.com/api)
