...
Keep StatusPro inventory synchronized with an external warehouse management system.
...
from statuspro_public_api_client.api.inventory import get_all_inventory_point
from statuspro_public_api_client.api.variant import get_all_variants
...
async def sync_inventory_from_warehouse(warehouse_data: list[dict[str, Any]]) -> dict[str, int]:
    ...
    async with StatusProClient() as client:
        ...
        from statuspro_public_api_client.api.stock_adjustment import create_stock_adjustment
        from statuspro_public_api_client.models import CreateStockAdjustmentRequest
        ...
        for item in warehouse_data:
            ...
            if not variant:
                print(f"Warning: SKU {sku} not found in StatusPro")
                stats["skipped"] += 1
                continue
...
async def get_low_stock_alerts(threshold: int = 10) -> list[dict[str, Any]]:
    ...
    async with StatusProClient() as client:
        ...
        from statuspro_public_api_client.api.variant import get_variant
...
from statuspro_public_api_client import StatusProClient
from statuspro_public_api_client.api.inventory import get_all_negative_stock
from statuspro_public_api_client.utils import unwrap_data
...
from statuspro_public_api_client.models import CreateSalesOrderRequest
...
from statuspro_public_api_client import StatusProClient
from statuspro_public_api_client.api.manufacturing_order import get_all_manufacturing_orders
from statuspro_public_api_client.utils import unwrap_data
...
from statuspro_public_api_client import StatusProClient
from statuspro_public_api_client.api.manufacturing_order import make_to_order_manufacturing_order
from statuspro_public_api_client.models import MakeToOrderManufacturingOrderRequest
...
async def retry_with_backoff(
    ...
    Note: Network errors and 429/5xx are already handled by StatusProClient.
...
async def get_products_with_fallback() -> list[Any]:
    ...
    """
    try:
        async with StatusProClient(timeout=10.0) as client:
...
Handle incoming webhook events from StatusPro.
...
from statuspro_public_api_client import StatusProClient
from statuspro_public_api_client.api.sales_order import get_sales_order


app = Flask(__name__)
WEBHOOK_SECRET = "your-webhook-secret"  # From StatusPro webhook settings


def verify_webhook_signature(payload: bytes, signature: str) -> bool:
    """
    Verify webhook signature from StatusPro.

    Args:
        payload: Raw request body bytes
        signature: X-StatusPro-Signature header value
...
@app.route("/webhooks/statuspro", methods=["POST"])
def handle_statuspro_webhook():
    """Handle incoming webhooks from StatusPro."""
    # Verify signature
    signature = request.headers.get("X-StatusPro-Signature", "")
...
Periodically sync data between StatusPro and external systems.
...
def setup_logging(level: int = logging.INFO) -> logging.Logger:
    ...
    logger = logging.getLogger("statuspro_integration")
...
async def fetch_products_with_logging() -> list[Any]:
    ...
    try:
        async with StatusProClient(logger=logger) as client:
            logger.debug("StatusProClient initialized")
...
# Instrument httpx (used by StatusProClient)
...
async def fetch_all_orders_efficiently(
    ...
    Returns:
        List of all orders
    """
    async with StatusProClient(max_pages=max_pages or 1000) as client:
...
async def parallel_data_enrichment(order_ids: list[int]):
    ...
    """
    from statuspro_public_api_client.api.sales_order import get_sales_order
    from statuspro_public_api_client.api.customer import get_all_customers
...
from statuspro_public_api_client import StatusProClient
from statuspro_public_api_client.api.product import get_all_products
from statuspro_public_api_client.models import Product
...
async def test_product_fetch_with_mock():
    ...
    with patch(
        'statuspro_public_api_client.api.product.get_all_products.asyncio_detailed',
        new=AsyncMock(return_value=mock_response)
    ):
        async with StatusProClient(api_key="test-key", base_url="https://test.api") as client:
            response = await get_all_products.asyncio_detailed(client=client)
...
@pytest.fixture
async def statuspro_client():
    """Fixture providing StatusProClient for integration tests."""
    async with StatusProClient() as client:
        yield client
...
@pytest.mark.asyncio
async def test_fetch_products_integration(statuspro_client):
    """Integration test for fetching products."""
    response = await get_all_products.asyncio_detailed(client=statuspro_client)
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_pagination_integration(statuspro_client):
    """Integration test for pagination behavior."""
    # Fetch with small page size
    response = await get_all_products.asyncio_detailed(
        client=statuspro_client,
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_retry_on_network_error(statuspro_client):
...
class TestHelpers:
    """Helper utilities for testing StatusPro integrations."""

    @staticmethod
    def create_mock_client() -> StatusProClient:
        """Create a mock StatusProClient for testing."""
        return StatusProClient(
            api_key="test-api-key",
            base_url="https://test.app.orderstatuspro.com/api/v1"
        )
...
async def test_with_helpers():
    ...
    assert client._base_url == "https://test.app.orderstatuspro.com/api/v1"
...
- Check the [API Documentation](https://help.orderstatuspro.com/api)
- Review [Client Guide](guide.md) for client features
- See
  [examples directory](https://github.com/dougborg/statuspro-openapi-client/tree/main/examples)
