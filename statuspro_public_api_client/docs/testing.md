...
- **External consistency**: Validation against official StatusPro documentation
...
- Validates against comprehensive documentation from developer.orderstatuspro.com
...
**`test_statuspro_client.py`** - Custom client implementation and transport layer\
...
- **Real API tests**: Require `STATUSPRO_API_KEY` in `.env` file
...
uv run pytest --cov=statuspro_public_api_client --cov-report=json -m 'not docs'
uv run poe analyze-coverage

# Generate HTML report for detailed inspection
uv run pytest --cov=statuspro_public_api_client --cov-report=html -m 'not docs'
...
See [Issue #31](https://github.com/dougborg/statuspro-openapi-client/issues/31) for
detailed improvement plan targeting:

- `statuspro_client.py`: 85% → 90%+ (add edge case tests)
