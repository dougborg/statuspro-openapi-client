...
This script creates all 24 issues for the StatusPro MCP Server implementation
...
Set up uv workspace configuration to enable monorepo management for both `statuspro-openapi-client` and `statuspro-mcp-server` packages.

## Acceptance Criteria

- [ ] Add `[tool.uv.workspace]` to root `pyproject.toml`
- [ ] Configure workspace members: `["statuspro_public_api_client", "statuspro_mcp_server"]`
...
- [ADR-010: StatusPro MCP Server](docs/adr/0010-statuspro-mcp-server.md)
...
""",
    ...
    Issue(
        number=2,
        title="Create statuspro_mcp_server package structure",
        body="""## Description

Create the directory structure and initial files for the `statuspro-mcp-server` package within the monorepo.

## Acceptance Criteria

- [ ] Create directory: `statuspro_mcp_server/src/statuspro_mcp/`
- [ ] Create subdirectories: `tools/`, `resources/`, `prompts/`
- [ ] Create `__init__.py` files in all directories
- [ ] Create `statuspro_mcp_server/pyproject.toml` with dependencies
- [ ] Create `statuspro_mcp_server/README.md` with installation instructions
- [ ] Create `statuspro_mcp_server/tests/` directory
...
version = "0.1.0"
description = "MCP server for StatusPro Manufacturing ERP"
...
- [ADR-010: Architecture](docs/adr/0010-statuspro-mcp-server.md#architecture)
""",
        labels=["infrastructure", "mcp-server", "p0-critical"],
        milestone="MCP Server v0.1.0 MVP",
        estimate_hours="2-3h",
...
Create the core FastMCP server with environment-based authentication and StatusProClient initialization.

## Acceptance Criteria

- [ ] Create `server.py` with FastMCP initialization
- [ ] Implement environment-based authentication (STATUSPRO_API_KEY, STATUSPRO_BASE_URL)
- [ ] Initialize StatusProClient with proper error handling
...
def get_statuspro_client() -> StatusProClient:
    api_key = os.getenv("STATUSPRO_API_KEY")
    if not api_key:
        raise ValueError("STATUSPRO_API_KEY environment variable is required")

    base_url = os.getenv("STATUSPRO_BASE_URL", "https://app.orderstatuspro.com/api/v1")
    return StatusProClient(api_key=api_key, base_url=base_url)
```
...
- [StatusProClient Guide](docs/STATUSPRO_CLIENT_GUIDE.md)
""",
...
- [ ] Use StatusProClient to fetch inventory data
...
- [ ] Write integration test (requires STATUSPRO_API_KEY)
...
async def check_inventory(sku: str, ctx: Context) -> InventoryStatus:
    ...
    # Implementation using StatusProClient
...
- [API: get_all_inventory_point](statuspro_public_api_client/api/inventory/)
""",
...
- [API: Products](statuspro_public_api_client/api/product/)
""",
