...
# Domain groupings for StatusPro. The spec is small (18 schemas) so only two
...
def add_base_entity_extra_ignore(classes: list[ClassInfo]) -> list[ClassInfo]:
    ...
    from the StatusPro API, while request models keep extra='forbid' from the base.
    See: https://github.com/dougborg/statuspro-openapi-client/issues/295
    """
    fixed_classes = []
    for cls in classes:
...
def generate_module_imports(
    ...
    for imp in imports:
        ...
        if imp.is_from_import and imp.module and "StatusProPydanticBase" in imp.names:
            continue
...
def generate_auto_registry(
    ...
    for py_file in attrs_models_dir.glob("*.py"):
        ...
        for match in re.finditer(class_pattern, content):
            ...
            module_path = f"statuspro_public_api_client.models.{py_file.stem}"
...
def format_code(workspace_path: Path) -> None:
    ...
    pydantic_dir = workspace_path / "statuspro_public_api_client" / "models_pydantic"
...
def main() -> None:
    ...
    attrs_models_dir = workspace_path / "statuspro_public_api_client" / "models"
    auto_registry_path = (
        workspace_path
        / "statuspro_public_api_client"
        / "models_pydantic"
