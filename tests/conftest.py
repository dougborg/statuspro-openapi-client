"""Test configuration and fixtures for the StatusPro OpenAPI Client test suite."""
...
def mock_api_credentials():
    ...
    return {
        ...
        "base_url": "https://api.statuspro.test",  # .test TLD reserved for testing (RFC 6761)
    }


@pytest.fixture
def statuspro_client(mock_api_credentials):
    """Create a StatusProClient for testing."""
    return StatusProClient(**mock_api_credentials)


@pytest.fixture
def statuspro_client_limited_pages(mock_api_credentials):
    """Create a StatusProClient with limited pagination for testing."""
    return StatusProClient(max_pages=5, **mock_api_credentials)
...
@pytest.fixture
def statuspro_client_with_mock_transport(mock_api_credentials, mock_transport):
    """Create a StatusProClient with MockTransport for testing."""
    # Create a StatusProClient
    client = StatusProClient(**mock_api_credentials)
...
def setup_test_env(monkeypatch, request):
    ...
    if is_real_api_test:
        ...
        if not os.getenv("STATUSPRO_API_KEY"):
            monkeypatch.setenv("STATUSPRO_API_KEY", "test-key")
        if not os.getenv("STATUSPRO_BASE_URL"):
            monkeypatch.setenv(
        ...
        monkeypatch.setenv("STATUSPRO_API_KEY", "test-key")
        monkeypatch.setenv("STATUSPRO_BASE_URL", "https://api.statuspro.test")
