...
import pytest
from statuspro_mcp.logging import (
    add_trace_id,
    clear_trace_id,
...
def test_setup_logging_from_env():
    ...
    with patch.dict(
        os.environ,
        {"STATUSPRO_MCP_LOG_LEVEL": "WARNING", "STATUSPRO_MCP_LOG_FORMAT": "text"},
    ):
        setup_logging()
...
def test_logging_env_var_precedence():
    ...
    with patch.dict(
        os.environ,
        {"STATUSPRO_MCP_LOG_LEVEL": "ERROR", "STATUSPRO_MCP_LOG_FORMAT": "text"},
    ):
        setup_logging()
