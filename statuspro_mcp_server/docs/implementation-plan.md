...
1. **`search_variants`** - Search across all item types (Products, Materials, Variants,
   ...
   - Output: `SearchVariantsResponse` with `items: list[VariantSearchResult]`,
     `total_count: int`
   - Returns: Pydantic domain models (StatusProVariant, StatusProProduct, etc.)
...
1. **`get_variant_details`** - Get detailed info for a specific SKU
   ...
   - Input: `sku: str`
   - Output: `VariantDetailsResponse` with full StatusProVariant + stock info + BOM info
...
4. **`create_product`** - Create new product with variants
   ...
   - Output: `CreateProductResponse` with created StatusProProduct + warnings +
     next_actions
...
1. **`create_material`** - Create new material
   ...
   - Output: `CreateMaterialResponse` with created StatusProMaterial + warnings +
     next_actions
...
state of your StatusPro system. Each resource stays under the 1MB size limit through
...
- **Size Management**:
  ...
  - Use `statuspro://inventory/items?cursor={next_cursor}` for next page
...
- **Data**:
  ```json
  {
    ...
    "pagination": {
      ...
      "next_uri": "statuspro://inventory/items?cursor=eyJpZCI6MTAwfQ=="
    },
    "alerts": {
      "low_stock_count": 15,
...
- **URI**: `statuspro://inventory/stock-movements`
- **Type**: Dynamic resource (refreshes every 5 minutes)
...
- **Data**:
  ```json
  {
    ...
    "pagination": {
      ...
      "next_uri": "statuspro://inventory/stock-movements?cursor=eyJ0aW1lc3RhbXAiOiIyMDI1LTAxLTE0VDEwOjMwOjAwWiJ9"
    },
    "next_actions": [
...
- **URI**: `statuspro://inventory/stock-adjustments`
- **Type**: Dynamic resource (refreshes every 5 minutes)
...
- **Data**:
  ```json
  {
    ...
    "pagination": {
      ...
      "next_uri": "statuspro://inventory/stock-adjustments?cursor=eyJpZCI6NDU1fQ=="
    },
    "next_actions": [
...
- **URI**: `statuspro://manufacturing-orders`
- **Type**: Dynamic resource (refreshes every 5 minutes)
...
- **Example**:
  ...
  async def search_variants(
      ...
      Returns:
          ...
          - items: List of domain models (StatusProVariant/StatusProProduct/StatusProMaterial)
      ...
      Related Resources:
          - statuspro://dashboard/inventory - See inventory overview first
          - statuspro://variant/{sku} - Get detailed info after search
...
async def create_purchase_order(
    ...
    Step 2: Call with confirm=True
        - Creates the actual PO in StatusPro
...
- Implement 6 resources:
  - **Inventory**: `statuspro://inventory/items`, `statuspro://inventory/stock-movements`,
    `statuspro://inventory/stock-adjustments`
  - **Orders**: `statuspro://sales-orders`, `statuspro://purchase-orders`,
    `statuspro://manufacturing-orders`
