# ADR-0010: StatusPro MCP Server for Claude Code Integration
...
The StatusPro OpenAPI Client provides a comprehensive Python SDK for interacting with the
StatusPro Manufacturing ERP API. However, using this client within Claude Code currently
...
interface. By creating a StatusPro MCP server, we can:

1. **Enable Natural Language Interactions**: Users can interact with their StatusPro
...
1. **Leverage Existing Client**: Build on top of our well-tested StatusProClient with its
...
- MCP Python SDK (`mcp.server.fastmcp`) provides FastMCP class for rapid server
  development
- Our StatusProClient already handles authentication, retries, rate limiting, and
  pagination
...
- Users want quick access to StatusPro data without writing integration code
...
- Lower the barrier to entry for StatusPro API automation
- Expand the client's ecosystem beyond pure Python usage

## Decision

We will create a `statuspro-mcp-server` as a **separate package within this repository**
(monorepo with uv workspace), implementing the Model Context Protocol to expose StatusPro
functionality to Claude Code and other MCP clients. The MCP server will depend on
`statuspro-openapi-client` and serve as both a production-ready tool and a reference
...
```
statuspro-openapi-client/                  # Repository root
├── pyproject.toml                      # Workspace configuration
├── uv.lock                             # Unified lockfile
├── statuspro_public_api_client/           # Client library package
│   └── ...                             # (existing structure)
...
# Root pyproject.toml
[tool.uv.workspace]
members = ["statuspro_public_api_client", "statuspro_mcp_server"]
...
version = "0.1.0"
dependencies = [
    "statuspro-openapi-client",  # Uses workspace version automatically
    "mcp>=1.0.0",
    "pydantic>=2.0.0",
]
```
...
- Direct database access (only via StatusPro API)
...
1. **Lower Barrier to Entry**: Users can access StatusPro without learning the API
...
1. **Leverages Existing Work**: Built on battle-tested StatusProClient with 96% test
...
1. **Testing Requirements**: Need integration tests that run against real StatusPro API (or
   mock server)
...
Create `statuspro-mcp-server` as a completely separate repository that depends on
`statuspro-openapi-client` from PyPI.
...
Implement as a Claude Code skill (`.claude/skills/statuspro.md`) instead of an MCP server.
...
Add MCP server functionality directly to `statuspro-openapi-client` as an optional feature
activated with `pip install statuspro-openapi-client[mcp]`.
...
- [GitHub Issue #XX: Create StatusPro MCP Server](#) <!-- TODO: Create issue -->
...
- Write unit tests with mocked StatusProClient
...
- Integration testing with real StatusPro API
...
### 1. Naming: `statuspro-mcp-server`

**Decision**: Use `statuspro-mcp-server` as the package name.

**Rationale**:

- Follows standard pattern: `{service}-mcp-server` (e.g., `postgres-mcp-server`)
- Clear that it's a server, not just an integration
- Sorts well in package listings (statuspro prefix keeps it near `statuspro-openapi-client`)
...
dependencies = [
    "statuspro-openapi-client>=0.21.0,<0.23.0",  # Compatible minor versions
    "mcp>=1.0.0",
]
```
...
**Decision**: Single StatusPro instance per server process in Phase 1. Users run multiple
server instances for multiple StatusPro environments.
...
{
  "mcpServers": {
    "statuspro-production": {
      "command": "uvx",
      "args": ["statuspro-mcp-server"],
      "env": {"STATUSPRO_API_KEY": "prod-key"}
    },
    "statuspro-staging": {
      "command": "uvx",
      "args": ["statuspro-mcp-server"],
      "env": {"STATUSPRO_API_KEY": "staging-key"}
    }
  }
}
```
...
1. ✅ Create `statuspro_mcp_server/` directory structure
1. ✅ Create initial `statuspro_mcp_server/pyproject.toml` with dependencies
