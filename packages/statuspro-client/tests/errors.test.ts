...
describe('Error Classes', () => {
  describe('StatusProError', () => {
    it('should create with message only', () => {
      const error = new StatusProError('Something went wrong');
      expect(error.message).toBe('Something went wrong');
      expect(error.name).toBe('StatusProError');
    ...
    it('should create with all options', () => {
      ...
      const error = new StatusProError('Bad request', {
        statusCode: 400,
        response,
        body,
      });
    ...
    it('should store cause', () => {
      ...
      const error = new StatusProError('Wrapped error', { cause });
...
describe('parseError', () => {
  ...
  it('should return generic StatusProError for other 4xx', () => {
    const response = new Response(null, { status: 404 });
    const body = { message: 'Product not found' };
    const error = parseError(response, body);
    expect(error).toBeInstanceOf(StatusProError);
