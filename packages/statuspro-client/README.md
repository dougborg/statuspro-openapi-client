...
[StatusPro API](https://app.orderstatuspro.com/api/v1) with automatic resilience. The
StatusPro API is a small REST surface for reading and updating the status of orders.
...
- **Rate-limit awareness** — respects 429 + `Retry-After` (falls back to exponential
  backoff; StatusPro doesn't surface the header today).
...
StatusPro does not expose `PUT`, `PATCH`, or `DELETE` — all mutations are POSTs.
