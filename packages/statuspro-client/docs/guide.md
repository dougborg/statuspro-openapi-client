...
The **StatusProClient** is the TypeScript/JavaScript client for the StatusPro Manufacturing
...
```typescript
// Automatically loads from STATUSPRO_API_KEY environment variable
...
Every API call through `StatusProClient` automatically includes retry logic:
...
const client = StatusProClient.withApiKey('your-api-key', {
  pagination: {
    ...
    defaultPageSize: 250, // Default: 250 (StatusPro API max)
  },
});
```
...
| `StatusProError`         | Other  | Base class for all errors  |
...
```typescript
interface StatusProClientOptions {
  // API key for authentication
  apiKey?: string;

  // Base URL (default: 'https://app.orderstatuspro.com/api/v1')
  baseUrl?: string;
...
const client = StatusProClient.withApiKey('sk-12345...');
```
...
const products = await (await StatusProClient.create()).get('/products');
const orders = await (await StatusProClient.create()).get('/sales_orders');
```
