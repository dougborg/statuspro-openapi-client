/**
 * StatusProClient - The main entry point for the StatusPro API client
 *
...
/**
 * Configuration options for StatusProClient
 */
export interface StatusProClientOptions {
  /**
  ...
  /**
   * Base URL for the StatusPro API.
   * Default: 'https://app.orderstatuspro.com/api/v1'
   */
  baseUrl?: string;
...
/**
 * Default base URL for StatusPro API
 */
const DEFAULT_BASE_URL = 'https://app.orderstatuspro.com/api/v1';
 ...
 * 2. STATUSPRO_API_KEY environment variable
 *
...
function resolveApiKey(explicitKey?: string): string {
  ...
  if (typeof process !== 'undefined' && process.env?.STATUSPRO_API_KEY) {
    return process.env.STATUSPRO_API_KEY;
  }

  throw new Error(
    'API key required. Provide via: apiKey option or STATUSPRO_API_KEY environment variable. ' +
...
/**
 * StatusProClient - The main StatusPro API client with automatic resilience
 *
 * Features:
 ...
 * ```typescript
 * const client = new StatusProClient({
 *   apiKey: 'your-api-key',
 *   retry: { maxRetries: 3 },
 ...
 * ```
 */
export class StatusProClient {
  private readonly apiKey: string;
  private readonly baseUrl: string;
  private readonly authenticatedFetch: typeof fetch;
  private readonly logger: NonNullable<StatusProClientOptions['logger']>;
  private readonly _sdkClient: Client;

  private constructor(apiKey: string, options: Omit<StatusProClientOptions, 'apiKey'> = {}) {
    this.apiKey = apiKey;
  ...
  /**
   * Create a new StatusProClient instance
   *
   * @param options - Client configuration options
   * @returns Promise resolving to a configured StatusProClient
   *
   * @example
   * ```typescript
   ...
   * ```
   */
  static async create(options: StatusProClientOptions = {}): Promise<StatusProClient> {
    const apiKey = resolveApiKey(options.apiKey);
    return new StatusProClient(apiKey, options);
  }

  /**
   * Create a StatusProClient synchronously with an explicit API key
   *
   * This is a convenience method when you already have the API key.
   * For automatic credential resolution, use `StatusProClient.create()`.
   *
   * @param apiKey - API key for authentication
   * @param options - Additional client options
   * @returns Configured StatusProClient instance
   *
   * @example
   * ```typescript
  ...
  static withApiKey(
    apiKey: string,
    options: Omit<StatusProClientOptions, 'apiKey'> = {}
  ): StatusProClient {
    return new StatusProClient(apiKey, options);
  }

  /**
   * Make an authenticated request to the StatusPro API
   *
