/**
 * Auto-pagination transport layer for StatusPro API
 *
 ...
 * - Uses 250 items per page (StatusPro's max) when no limit specified by caller
 ...
 *
 * Mirrors Python client's PaginationTransport pattern from statuspro_client.py
 */

/**
 * Pagination metadata returned from StatusPro API
 */
...
export interface PaginationConfig {
  ...
  maxItems?: number;
  /** Default page size when not specified. Default: 250 (StatusPro API max) */
...
export const DEFAULT_PAGINATION_CONFIG: PaginationConfig = {
  maxPages: 100,
  maxItems: undefined,
  defaultPageSize: 250, // StatusPro API max page size
};
 ...
 * const response = await paginatedFetch('https://app.orderstatuspro.com/api/v1/products');
 *
 * // Auto-paginate with custom limit per page
 * const custom = await paginatedFetch('https://app.orderstatuspro.com/api/v1/products?limit=100');
 *
 * // Disable auto-pagination with explicit page (ANY page value)
 * const page1 = await paginatedFetch('https://app.orderstatuspro.com/api/v1/products?page=1');
 * const page2 = await paginatedFetch('https://app.orderstatuspro.com/api/v1/products?page=2');
 * ```
 */
...
async function performPagination(
  ...
  // Get caller's limit or default to 250 (StatusPro's max) for efficiency
