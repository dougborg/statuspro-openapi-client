/**
 * Resilient transport layer for StatusPro API
 *
 * Provides automatic retry with exponential backoff, rate limiting awareness,
 * and proper handling of different HTTP methods.
 *
 * Mirrors Python client's RateLimitAwareRetry pattern from statuspro_client.py
 */
 ...
 * });
 *
 * const response = await resilientFetch('https://app.orderstatuspro.com/api/v1/products');
 * ```
 */
