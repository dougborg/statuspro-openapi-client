...
```bash
# Add to statuspro-openapi-client
cd statuspro_public_api_client
uv add httpx

# Add to statuspro-mcp-server
...
**statuspro-openapi-client** (independent):

- httpx, attrs, python-dateutil, etc.
- No dependency on MCP server

**statuspro-mcp-server** (depends on client):

```toml
[project]
dependencies = [
    "statuspro-openapi-client",  # Internal dependency
    "mcp>=1.0.0",
    # ...
]
```
