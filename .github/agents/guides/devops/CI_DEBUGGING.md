...
Error: Files would be reformatted:
  statuspro_mcp_server/src/statuspro_mcp/tools/inventory.py
```
...
```
statuspro_mcp_server/src/statuspro_mcp/tools/inventory.py:42:5: F841 Local variable `result` is assigned to but never used
```
...
```
statuspro_mcp_server/src/statuspro_mcp/tools/inventory.py:42: error: Incompatible return value type (got "str", expected "int")
```
...
```
WARNING - Could not find 'statuspro_mcp.tools.inventory' in docstrings
```
