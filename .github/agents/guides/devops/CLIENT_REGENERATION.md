...
- New endpoints added to `docs/statuspro-openapi.yaml`
...
git log --oneline docs/statuspro-openapi.yaml
...
- Fix errors in `docs/statuspro-openapi.yaml`
...
```bash
# Create backup
cp -r statuspro_public_api_client statuspro_public_api_client.backup

# Or commit current state
git add statuspro_public_api_client
...
# - statuspro_public_api_client/api/
# - statuspro_public_api_client/models/
...
git diff statuspro_public_api_client/

# Focus on API changes
git diff statuspro_public_api_client/api/

# Focus on model changes
git diff statuspro_public_api_client/models/
```
...
```
statuspro_public_api_client/statuspro_client.py  ✅ Preserved
statuspro_public_api_client/log_setup.py      ✅ Preserved
```
...
# Update custom code (statuspro_client.py) if needed
```
...
1. Check what changed: `git diff statuspro_public_api_client/models/`
...
**File:** `docs/statuspro-openapi.yaml`
...
```yaml
packageName: statuspro_public_api_client
projectName: statuspro-openapi-client
...
rm -rf statuspro_public_api_client
mv statuspro_public_api_client.backup statuspro_public_api_client

# Option 2: Git revert
git restore statuspro_public_api_client/
...
**Remember:** Generated code is read-only. Customizations go in `statuspro_client.py`!
