...
You are a specialized testing agent for the statuspro-openapi-client project. Your
...
- Use fixtures from `tests/conftest.py` (`mock_transport`, `statuspro_client_with_mock_transport`)
...
# Integration tests (requires STATUSPRO_API_KEY)
...
├── test_statuspro_client.py          # Client tests
...
```python
@pytest.mark.asyncio
async def test_get_products_success(statuspro_client_with_mock_transport):
    """Test successful product retrieval using the mock transport fixture."""
    # Arrange - fixture provides StatusProClient with MockTransport pre-configured
    client = statuspro_client_with_mock_transport
...
For custom response handlers, create a handler and wire it into a StatusProClient:
...
uv run pytest --cov=statuspro_public_api_client --cov-report=html
open htmlcov/index.html

# Check specific module
uv run pytest --cov=statuspro_public_api_client.helpers tests/
...
# Add your API key
echo "STATUSPRO_API_KEY=your-key-here" >> .env
