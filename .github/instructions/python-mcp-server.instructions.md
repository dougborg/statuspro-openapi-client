...
## StatusPro MCP Server Patterns

### ServerContext Pattern (ADR-010)

- Use `get_services()` to access StatusProClient and other services
- Never instantiate StatusProClient directly in tools
...
async def risky_operation(input: str) -> str:
    ...
    try:
        services = get_services()
        result = await perform_operation(services.statuspro_client, input)
